...
bool CSocekt::Initialize()
{
    ...
    {
        ngx_log_stderr(WSAGetLastError(), "WSAStartup failed");
        return false;
    }
    ReadConf();  //读配置项
...
{
    //发消息互斥量初始化
    InitializeCriticalSection(&m_sendMessageQueueMutex);
    //连接相关互斥量初始化
    InitializeCriticalSection(&m_connectionMutex);
    //连接回收队列相关互斥量初始化
    InitializeCriticalSection(&m_recyconnqueueMutex);
    //和时间处理队列有关的互斥量初始化
    InitializeCriticalSection(&m_timequeueMutex);
    ...
    m_semEventSendQueue = CreateSemaphore(NULL, 0, 10000, NULL);
    if (m_semEventSendQueue == NULL)
    {
        ngx_log_stderr(GetLastError(), "CreateSemaphore failed");
        return false;
    }
    ...
    pSendQueue->_Handle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ServerSendQueueThread, pSendQueue, 0, NULL);
    if (pSendQueue->_Handle == NULL)
    {
        ngx_log_stderr(GetLastError(), "CreateThread failed");
        return false;
    }
    if(err != 0)
    {
    ...
    pRecyconn->_Handle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ServerRecyConnectionThread, pRecyconn, 0, NULL);
    if (pRecyconn->_Handle == NULL)
    {
        ngx_log_stderr(GetLastError(), "CreateThread failed");
        return false;
    }
    ...
    if(m_ifkickTimeCount == 1)  //是否开启踢人时钟，1：开启   0：不开启
    {
        ...
        pTimemonitor->_Handle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ServerTimerQueueMonitorThread, pTimemonitor, 0, NULL);
    if (pTimemonitor->_Handle == NULL)
    {
        ngx_log_stderr(GetLastError(), "CreateThread failed");
        return false;
    }
...
void CSocekt::Shutdown_subproc()
{
    ...
    if(ReleaseSemaphore(m_semEventSendQueue, 1, NULL) == FALSE)  //让ServerSendQueueThread()流程走下来干活
    {
    ...
    {
        WaitForSingleObject((*iter)->_Handle, INFINITE); //等待线程终止
        CloseHandle((*iter)->_Handle); //关闭线程句柄
    }
    ...
    //(4)多线程相关    
    DeleteCriticalSection(&m_connectionMutex);          //连接相关临界区释放
    DeleteCriticalSection(&m_sendMessageQueueMutex);    //发消息临界区释放    
    DeleteCriticalSection(&m_recyconnqueueMutex);       //连接回收队列临界区释放
    DeleteCriticalSection(&m_timequeueMutex);           //时间处理队列临界区释放
    CloseHandle(m_semEventSendQueue);                   //关闭信号量句柄
}
...
void CSocekt::zdClosesocketProc(lpngx_connection_t p_Conn)
{
    ...
    if(p_Conn->fd != -1)
    {   
        closesocket(p_Conn->fd); //这个socket关闭，关闭后epoll就会被从红黑树中删除，所以这之后无法收到任何epoll事件
        p_Conn->fd = -1;
    }
...
bool CSocekt::TestFlood(lpngx_connection_t pConn)
{
	...
	iCurrTime = GetTickCount64(); //取得当前时间（毫秒）
...
int CSocekt::ngx_epoll_init()
{
    ...
    m_epollhandle = (int)CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, 0); if (m_epollhandle == 0) { ngx_log_stderr(GetLastError(), "CreateIoCompletionPort failed"); exit(2); }
    if (m_epollhandle == -1) 
    {
...
//对epoll事件的具体操作
//返回值：成功返回1，失败返回-1；
int CSocekt::ngx_iocp_oper_event(
                        int                fd,               //句柄，一个socket
                        ...
                        )
{
    ...
    {
        // 将socket与完成端口关联
        HANDLE hIOCP = CreateIoCompletionPort((HANDLE)fd, (HANDLE)m_epollhandle, (ULONG_PTR)pConn, 0);
        if (hIOCP == NULL)
        {
            ngx_log_stderr(GetLastError(), "CreateIoCompletionPort failed");
            return -1;
        }
        // 初始化重叠结构并投递接收请求
        memset(&pConn->overlapped, 0, sizeof(OVERLAPPED));
        DWORD bytesRecv = 0;
        DWORD flags = 0;
        WSABUF wsaBuf;
        wsaBuf.buf = pConn->precvbuf;
        wsaBuf.len = pConn->irecvlen;
        if (WSARecv(fd, &wsaBuf, 1, &bytesRecv, &flags, &pConn->overlapped, NULL) == SOCKET_ERROR)
        {
            if (WSAGetLastError() != WSA_IO_PENDING)
            {
                ngx_log_stderr(WSAGetLastError(), "WSARecv failed");
                return -1;
            }
        }
    }
    ...
    {
        // 修改事件，对于IOCP通常是重新投递请求
        if (flag & EPOLLOUT)
        {
            DWORD bytesSent = 0;
            WSABUF wsaBuf;
            wsaBuf.buf = pConn->psendbuf;
            wsaBuf.len = pConn->isendlen;
            if (WSASend(fd, &wsaBuf, 1, &bytesSent, 0, &pConn->overlapped, NULL) == SOCKET_ERROR)
            {
                if (WSAGetLastError() != WSA_IO_PENDING)
                {
                    ngx_log_stderr(WSAGetLastError(), "WSASend failed");
                    return -1;
                }
            }
        }
...
int CSocekt::ngx_iocp_process_events(int timer) 
{   
    ...
    DWORD bytesTransferred; ULONG_PTR completionKey; LPOVERLAPPED overlapped; int events = 0; while (GetQueuedCompletionStatus((HANDLE)m_epollhandle, &bytesTransferred, &completionKey, &overlapped, timer)) { events++; lpngx_connection_t p_Conn = (lpngx_connection_t)completionKey; if (p_Conn && overlapped) { // 处理完成事件 if (p_Conn->rhandler) (this->*p_Conn->rhandler)(p_Conn, bytesTransferred); } } if (GetLastError() != WAIT_TIMEOUT) ngx_log_stderr(GetLastError(), "GetQueuedCompletionStatus failed");    
...
{    
    ...
    while(g_stopEvent == 0) //不退出
    {
        ...
        if(WaitForSingleObject(pSocketObj->m_semEventSendQueue, INFINITE) != WAIT_OBJECT_0)
        {
            //失败？及时报告，其他的也不好干啥
        ...
        {
            EnterCriticalSection(&pSocketObj->m_sendMessageQueueMutex); //进入临界区
            ...
            LeaveCriticalSection(&pSocketObj->m_sendMessageQueueMutex); //离开临界区
